...
from webapp.www.apis import Page,APIPermissionError,APIError,APIValueError, APIresourceNotFundError
...
def api_comments_delete(id,request):
	...
	if c is None:
		return APIresourceNotFundError('Comment')
	yield from c.remove()
	return dict(id=id)
...
def api_create_comment(id,request,*,content):
	...
	if user is 	None:
		raise APIValueError('Please signin first')
	if not content or not content.strip():
		raise APIValueError('Please content first')
	blog = yield from Blog.find(id)
	if blog is None:
		raise APIresourceNotFundError('not find Blog')
